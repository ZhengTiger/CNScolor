...
```{r}
choices2 <- c(1:length(Image_url2))
names(choices2) <- paste("第",choices2,"期")
selectInput('number2', 'Color Palette', choices2)
```

```{r}
numericInput('colornum2', 'Color Number', value = 4, min = 1, max = 256)
```
...
HEX_select2 <- reactive(HEX_color2[[as.numeric(input$number2)]])
RGB_select2 <- reactive(RGB_color2[[as.numeric(input$number2)]])
color_df2 <- reactive(data.frame(
  "HEX" = HEX_select2()[1:input$colornum2],
  "RGB" = RGB_select2()[1:input$colornum2]
))
...
#    updateNumericInput(inputId = "colornum", max = length(HEX_select()), value=4)
#  }else{
    freezeReactiveValue(input, "colornum2")
    updateNumericInput(inputId = "colornum2", max = length(HEX_select2()),
                       value=length(HEX_select2()))
#  }
})

observeEvent(input$colornum2, {
  if (input$colornum2 > length(HEX_select2())) {
    freezeReactiveValue(input, "colornum2")
    updateNumericInput(inputId = "colornum2", max = length(HEX_select2()),
                       value=length(HEX_select2()))
  }
...
output$clip_HEX2 <- renderUI({
  rclipButton(
    inputId = "clipbtn_HEX2",
    label = "HEX Copy",
    clipText = HEX_select2()[1:input$colornum2], 
    icon = icon("clipboard"),
    tooltip = "Successfully Copied!",
    placement = "auto",
    options = list(delay = list(show = 100, hide = 100), trigger = "click"),
...
output$clip_RGB2 <- renderUI({
  rclipButton(
    inputId = "clipbtn_RGB2",
    label = "RGB Copy",
    clipText = RGB_select2()[1:input$colornum2], 
    icon = icon("clipboard"),
    tooltip = "Successfully Copied!",
    placement = "auto",
    options = list(delay = list(show = 100, hide = 100), trigger = "click"),
...
output$picture2 <- renderUI({
  tags$img(src = Image_url2[as.numeric(input$number2)])
  })
```
...
output$line_plot2 <- renderPlot({
  if (input$colornum2>=1 | input$colornum2<=length(HEX_select2())){
    Value <- c()
    for (i in 1:input$colornum2){
      Value <- c(Value, runif(7, min=i-0.5, max=i+0.5))
    }
    data <- data.frame(
      'Date' = rep(c('Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'), input$colornum2),
      'Value' = Value,
      'Type' = rep(paste("color",1:input$colornum2), each=7)
    )
      data$Date <- factor(data$Date, levels = c('Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'))
      data$Type <- factor(data$Type, levels = paste("color",1:input$colornum2))
      ...
      ggplot(data, aes(x=Date, y=Value, color=Type, fill=Type, group=Type)) +
        ...
        theme(panel.grid.major.y = element_line(),
              legend.position = "top", legend.title = element_blank(), 
              legend.margin = margin(0,35,0,0)) +
        scale_fill_manual(values=HEX_select2()) +
        scale_color_manual(values=HEX_select2()) +
        guides(fill=guide_legend(ncol = 4, byrow = TRUE))
    }
  })
```
...
output$scatter_plot2 <- renderPlot({
  ...
  data <- data.frame(
    'X' = X,
    'Y' = Y,
    'Type' = rep(paste("color",1:input$colornum2), each=10)
  )
  data$Type <- factor(data$Type, levels = paste("color",1:input$colornum2))
...
output$bar_plot2 <- renderPlot({
  X <- as.character(1:input$colornum2)
  Y <- sample(1:input$colornum2, input$colornum2)
  data <- data.frame(
    'X' = X,
    'Y' = Y,
    'Type' = paste('color', 1:input$colornum2)
  )
  data$X <- factor(data$X, levels = data$X)
  data$Type <- factor(data$Type, levels = paste('color', 1:input$colornum2))
